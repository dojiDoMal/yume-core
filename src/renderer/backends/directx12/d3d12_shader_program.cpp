...
bool D3D12ShaderProgram::link() { return createPipeline(); }

bool D3D12ShaderProgram::createPipeline() {
    auto device = backend->getDevice();

    // Root signature layout (must match the HLSL registers):
    //   [0] CBV b0  Matrices     (vertex)          -> "ModelViewProjection"
    //   [1] CBV b1  MaterialData (pixel)           -> "MaterialData"
    //   [2] CBV b2  LightData    (pixel)           -> "LightData"
    //   [3] SRV t0  StructuredBuffer<float4x4>     -> per-instance model matrices
    //
    // The instance StructuredBuffer (t0) mirrors the OpenGL SSBO (binding 3):
    // it lets flat.vxs index instanceModels[SV_InstanceID] for GPU instancing.
    ...
    rootParams[3].Descriptor.ShaderRegister = 0; // t0
    ...
    ID3DBlob* signature;
    ID3DBlob* error;
    if (FAILED(D3D12SerializeRootSignature(&rootSigDesc, D3D_ROOT_SIGNATURE_VERSION_1, &signature,
                                           &error))) {
    ...
    // Match the OpenGL backend, which never enables GL_CULL_FACE: both triangle
    // faces are drawn, so back faces are never culled. This keeps rendering
    // identical across backends (and avoids "seeing through" meshes when the
    // winding differs). FrontCounterClockwise mirrors GL's default CCW winding.
    ...
    // Must match the RTV format (UNORM or UNORM_SRGB, per project.conf srgb).
    psoDesc.RTVFormats[0] = backend->getRtvFormat();
...
void D3D12ShaderProgram::setUniformBuffer(const char* name, const void* data, size_t size) {
    ...
    if (buffer == nullptr) {
        ...
        backend->getDevice()->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &bufferDesc,
...
void* D3D12ShaderProgram::getHandle() const { return pipelineState; }

bool D3D12ShaderProgram::isValid() const { return pipelineState != nullptr; }
