...
bool VulkanShaderProgram::link() { return createPipeline(); }
...
bool VulkanShaderProgram::createPipeline() {
    ...
    // Match the OpenGL backend, which never enables GL_CULL_FACE: draw both
    // faces so winding differences never make a mesh look see-through.
    ...
    if (vkCreatePipelineLayout(backend->getDevice(), &pipelineLayoutInfo, nullptr,
    ...
    return vkCreateGraphicsPipelines(backend->getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo,
...
void VulkanShaderProgram::setUniformBuffer(const char* name, const void* data, size_t size) {
    // Route named blocks (MaterialData/LightData/ModelViewProjection) to the
    // backend's host-visible buffers. Material::setBaseColor/applyLight call
    // this with "MaterialData"/"LightData".
    if (backend)
        backend->setBufferDataImpl(name, data, size);
}

void* VulkanShaderProgram::getHandle() const { return (void*)pipeline; }

bool VulkanShaderProgram::isValid() const { return pipeline != VK_NULL_HANDLE; }
